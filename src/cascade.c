...
/**
 * Generate explanation of what rule changed
 */
...
/**
 * Merge actions: new rule's actions override/extend previous
 */
...
/**
 * Analyze cascade: how rules stack for a window
 */
struct cascade_analysis *cascade_analyze(
    ...
    if (analysis->step_count == 0) {
        ...
        for (size_t i = 0; i < analysis->step_count && i < 3; i++) {
            if (i > 0) strcat(analysis->summary, " → ");
            strcat(analysis->summary, 
                   ruleset->rules[analysis->steps[i].rule_index].display_name);
        }
        if (analysis->step_count > 3) {
            strcat(analysis->summary, " + more");
        }
    }
...
/**
 * Get explanation of what rule contributes
 */
char *cascade_explain_rule(
    ...
    if (rule->actions.workspace) {
        snprintf(actions, sizeof(actions), "workspace=%s", rule->actions.workspace);
        any = 1;
    }
    if (rule->actions.tag) {
        if (any) strcat(actions, ", ");
        snprintf(actions + strlen(actions), sizeof(actions) - strlen(actions), 
                 "tag=%s", rule->actions.tag);
        any = 1;
    }
    if (rule->actions.float_set) {
        if (any) strcat(actions, ", ");
        strcat(actions, rule->actions.float_val ? "float" : "nofloat");
        any = 1;
    }
    if (rule->actions.center_set) {
        if (any) strcat(actions, ", ");
        strcat(actions, rule->actions.center_val ? "center" : "nocenter");
        any = 1;
    }
    if (rule->actions.opacity) {
        if (any) strcat(actions, ", ");
        snprintf(actions + strlen(actions), sizeof(actions) - strlen(actions), 
                 "opacity=%s", rule->actions.opacity);
        any = 1;
    }
